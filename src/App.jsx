...
export default function App() {
  ...
  function getStampRarity(stamp) {
    if (!stamp) return 'common'
    const raw = String(stamp.rarity || stamp.type || '').toLowerCase()
    ...
    if (activeDropWindows?.[stamp.id]?.isLegendary) return 'legendary'
    if (activeDropWindows?.[stamp.id]?.isSecret) return 'secret'
    return 'common'
  }

  function showStampUnlock(stamp, method = 'qr-nfc') {
    if (!stamp) return
  ...
  async function collectNearbyGpsDropsAtLocation(currentLocation, options = {}) {
    ...
    if (newUnlockedIds.length) {
      ...
      showStampUnlock(firstUnlockedStamp, options.method || 'gps-pin-drop')
    }
  ...
  async function collectActiveStamp(method = 'manual') {
    ...
    if (!wasAlreadyCollected || method !== 'manual') {
      showStampUnlock(activeStamp, method)
    }
  }
  ...
  return (
    <main style={styles.screen}>
      ...
      {unlockCelebration && (
        <div style={styles.unlockOverlay} onClick={() => setUnlockCelebration(null)}>
          <div style={styles.unlockModal} onClick={(event) => event.stopPropagation()}>
            <p style={styles.unlockEyebrow}>⚡ EDM PASSPORT UNLOCK ⚡</p>
            <h1 style={styles.unlockTitle}>{unlockCelebration.title}</h1>
            <div style={styles.unlockStampFrame}>
              <Stamp stamp={unlockCelebration.stamp} collected isAdmin />
            </div>
            <h2 style={styles.unlockStampName}>{unlockCelebration.stamp.name}</h2>
            <p style={styles.unlockLocation}>{unlockCelebration.stamp.location || 'Festival Drop'}</p>
            <div style={styles.unlockMetaRow}>
              <span style={styles.unlockRarityBadge}>{unlockCelebration.rarity.toUpperCase()}</span>
              <span style={styles.unlockXpBadge}>+{unlockCelebration.stamp.xp || 100} XP</span>
            </div>
            <p style={styles.unlockMessage}>
              Added to your EDM Passport. This stamp is now revealed in your collection.
            </p>
            <button style={styles.mainButton} onClick={() => setUnlockCelebration(null)}>
              ADD TO PASSPORT
            </button>
          </div>
        </div>
...
const styles = {
  ...
  unlockOverlay: { position: 'fixed', inset: 0, zIndex: 1000, padding: 18, display: 'grid', placeItems: 'center', background: 'radial-gradient(circle at top, rgba(255,0,200,.34), transparent 38%), radial-gradient(circle at bottom, rgba(0,255,255,.26), transparent 40%), rgba(2,0,12,.88)', backdropFilter: 'blur(10px)' },
  unlockModal: { width: '100%', maxWidth: 390, padding: 22, borderRadius: 30, textAlign: 'center', color: 'white', background: 'linear-gradient(135deg, rgba(8,0,30,.98), rgba(75,0,130,.94), rgba(0,65,85,.92))', border: '1px solid rgba(103,232,249,.55)', boxShadow: '0 0 44px rgba(255,0,200,.36), 0 0 70px rgba(0,255,255,.22)', display: 'grid', gap: 10, animation: 'unlockPulse 900ms ease-out' },
  unlockEyebrow: { margin: 0, color: '#67e8f9', fontSize: 11, letterSpacing: '.16em', textTransform: 'uppercase', fontWeight: 900 },
  unlockTitle: { margin: 0, fontSize: 25, lineHeight: 1.05, fontWeight: 900, textShadow: '0 0 18px rgba(255,0,200,.65)' },
  unlockStampFrame: { margin: '8px auto', width: 128, height: 128, borderRadius: 999, display: 'grid', placeItems: 'center', background: 'radial-gradient(circle, rgba(255,255,255,.26), rgba(255,0,200,.20), rgba(0,255,255,.16))', border: '2px solid rgba(255,255,255,.50)', boxShadow: '0 0 38px rgba(255,0,200,.40), inset 0 0 25px rgba(255,255,255,.18)' },
  unlockStampName: { margin: '4px 0 0', fontSize: 24, fontWeight: 900 },
  unlockLocation: { margin: 0, color: '#cffafe', fontWeight: 800 },
  unlockMetaRow: { display: 'flex', justifyContent: 'center', gap: 8, flexWrap: 'wrap', marginTop: 4 },
  unlockRarityBadge: { padding: '7px 12px', borderRadius: 999, background: 'linear-gradient(90deg, #ff4fd8, #a855f7, #22d3ee)', color: '#020617', fontSize: 11, fontWeight: 900, letterSpacing: '.08em' },
  unlockXpBadge: { padding: '7px 12px', borderRadius: 999, background: 'linear-gradient(90deg, #fde047, #fb923c)', color: '#1f1300', fontSize: 11, fontWeight: 900, letterSpacing: '.08em' },
  unlockMessage: { margin: '4px 0', color: 'rgba(255,255,255,.82)', fontWeight: 800, lineHeight: 1.35 },
}
