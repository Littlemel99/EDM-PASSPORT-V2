...
test('Mission Control exposes one accessible Passport card action', () => {
  const dashboard = readFileSync(new URL('../Dashboard/FestivalDashboard.jsx', import.meta.url), 'utf8')
  assert.match(dashboard, /aria-label="Open festival passport"/)
  assert.match(dashboard, /OPEN PASSPORT/)
  assert.match(dashboard, /EDIT PASSPORT PROFILE/)
  assert.equal((dashboard.match(/EDIT PASSPORT PROFILE/g) || []).length, 1)
  assert.equal(dashboard.includes('title="My Passport"'), false)
  assert.ok(dashboard.indexOf('NEXT TARGET') < dashboard.indexOf('OPEN PASSPORT'))
})
